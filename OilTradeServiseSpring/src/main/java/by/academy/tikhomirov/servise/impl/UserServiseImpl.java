...
public class UserServiseImpl implements UserServise {
	...
	@Override
	public UserVO getAuthorizedUser(String login) {
		logger.info("Getting user with login: " + login + " :start");
		User user = new User();
		UserVO userVO = null;
		user = userRepository.getAuthorizedUser(login);
		if (user != null) {
			userVO = extract(user);
		} else {
			logger.debug("Can't get user with login: " + login + " :user doesn't exist");
		}
		logger.info("Getting user with login: " + login + " :complete");
		return userVO;
	}
